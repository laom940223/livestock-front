...
export const EditAnimal = ({onClose, isOpen, animals ,farmId, toUpdate } : { onClose: ()=>void, animals:Animal[] ,isOpen: boolean, farmId: number, toUpdate : number})=>{


    const defaultAnimal = animals.filter(animal => animal.id === toUpdate)[0]

   

  const [addMom, setAddMom ]= useState(()=>{   return animals.filter(animal=> animal.id === defaultAnimal.mother?.id).length  > 0 })
  const [addDad, setAddDad] = useState(()=>{   return animals.filter(animal=> animal.id === defaultAnimal.father?.id).length  > 0 })
    ...
    const {
      ...
      const  {mutate, isLoading } = useMutation<  ServerResponse<Animal>, AxiosError<ServerResponse<unknown>> ,unknown, { name: string, address: string}>({ 
        ...
        mutationFn: async ( variables )=>{
            ...
            const response = await  axios.put<ServerResponse<Animal>>(BASE_URL+`/animals/${toUpdate}`, variables, {
                headers:{
                    Authorization:getTokenString()
                }
            })
      ...
      const submitFunction = async (values:FieldValues)=>{
       ...
       const data=  {name: values.name, dob, breed_id: values.breed, sex: values.sex, farm_id:farmId, mother_id: addMom ?  values.mother : null, father_id: addDad ? values.father : null }
      ...
      const males = animals.filter( animal => animal.sex === "MALE" && dayjs().diff(dayjs(animal.dob),"y",false) > 2  )
      const females = animals.filter(animal=> animal.sex === "FEMALE" && dayjs().diff(dayjs(animal.dob),"y",false) > 2)

      

      // console.log(defaultAnimal.)
          //   console.log(animals.filter(animal=> animal.id === defaultAnimal.motherData?.id))
    //   console.log(animals.filter(animal=> animal.id === defaultAnimal.fatherData?.id))
            ...
            <Box sx={style}>
                ...
                <Controller
                    ...
                    <Controller
                        control={control}
                        name="dob"
                        defaultValue={dayjs(defaultAnimal.dob)}
                        render={({ field: { onChange,  value } }) => (
                        <DatePicker
                            label="Date of birth"
                            format="YYYY-MM-DD"
                    ...
                    <Controller
                              control={control}
                              name="sex"
                              defaultValue={defaultAnimal.sex}
                              render={({ field: { onChange,  value } }) => (
...

                    {
                        !isBreedLoading ?
                          ...
                          !isBreedError ? 
                              ...
                              control={control}
                              name="breed"
                              defaultValue={defaultAnimal.breed.id}
                              render={({ field: { onChange,  value } }) => (
